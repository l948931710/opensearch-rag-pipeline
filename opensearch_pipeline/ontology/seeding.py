...
- **已有 active 别名 → 跳过**（幂等：崩溃重跑/断点续跑天然安全）；
...
- **无任何候选 → 铸新对象 + canonical 别名**（标 confirmed_by='auto' 入抽检）。方向刻意保守：
  标题微差会多铸重复品（宁多建，steward 用 mark_duplicate 纠错），绝不静默合并。
- **低置信/多候选一律入 case，不铸新对象**（防"U8 货号逐条建品"式重复品繁殖）。

dry-run（CLI 默认）：零写库，用批内账本（planned 对象/别名/case）仿真批内先后效应——
同批两条同名记录，dry 与真跑同样报「铸 1 + auto 别名 1」。
"""
...
class SeedReport:
    ...
    def summary(self) -> str:
        ...
        return (f"[{mode}] 记录 {self.records}：跳过(已激活) {self.skipped_active} · "
                ...
                f"入工作台 case {self.cases_opened} · 错误 {self.errors}")
...
class _Sink:
    """播种写面 + 批内视图。dry-run 用 planned 账本仿真批内先后效应（零写库）；
    真跑直落 store（别名撞 uk → 视同已激活，幂等续跑）。"""

    def __init__(self, store, dry_run: bool):
        self._store = store
        self.dry = dry_run
    ...
    def alias(self, ns: str, raw: str, norm: str, target: str, *, method: str,
        ...
        if self.dry:
            ...
            return True
        try:
            self._store.insert_identifier(ns, raw, norm, target, method=method,
    ...
    def open_case(self, r: SeedRecord, norm: str, candidates: List[Candidate]) -> bool:
        ...
        case_id = self._store.upsert_case(
            ...
            evidence={"source": "seeding", "title": r.title, "attrs": r.attrs})
        for c in candidates:
...
def _decide(r: SeedRecord, sink: _Sink, tau: TauTable, report: SeedReport) -> None:
...
def seed_snapshot(store, source: Any, *, dry_run: bool = True,
                  ...
                  limit: Optional[int] = None) -> SeedReport:
    """跑一遍快照播种。单条失败只记 errors 不拖垮批（可断点重跑，幂等）。"""
    tau = tau_table or TauTable.from_env()
    sink = _Sink(store, dry_run)
