...
3. 保持简洁专业的语气
4. 仅当用户问的是**操作流程/步骤类**问题（答案是一串要依次执行的动作，如系统操作、办理手续）时，用分步骤方式回答：完整列出该流程的所有步骤与关键参数、不遗漏后续步骤，每步单独成段、以「**第N步**」开头（N 用阿拉伯数字，如 **第1步**），不要使用"1."、"①"等其他编号格式。若答案是**条款、分档规则、清单、禁令、处罚/奖惩标准**等非动作序列内容（如"如何划分/有哪些规定/罚多少"），不要套用「第N步」，改用条目符号或文档原有编号呈现——与该问题直接相关的条件、分档、例外、折算/豁免细则同样要完整覆盖，不因格式紧凑而省略；完整性不凌驾其他规则：上下文不支持的内容仍按规则2处理（不要为了显得全面而硬凑），规则8同样继续适用
...
_DOC_CITATION_TOKEN_PATTERN = re.compile(
    ...
    r'[\[【（(]\s*(?:见|参见|详见|引自|出自|来自|根据|依据)?\s*'  # 引导词可在括号内：「（见文档1、文档5）」
    r'文档\s*\d+(?:\s*[、,，/和与]\s*(?:文档\s*)?\d+)*\s*[\]】）)]'
    r'(?:\([^)\n]*\))?'
)
...
# 裸「来源：」行式：弱词只在「冒号后通篇仅为文档引用列表」时整行删（《标题》/文件名 +
# 顿号逗号空格，无任何其他实质内容）——2026-06-11 实测 LLM 会产出
# 「来源：《员工手册202108月.docx》、《A8休假请假管理标准.docx》」这种纯引用尾行，
# 它不含"文档N"也不带强标题词，曾穿透上面两式。"来源：内部口头通知…"等实质行不受影响。
_BARE_SOURCE_REFLIST_PATTERN = re.compile(
    r'^[ \t>*\-•·]*\*{0,2}来源\*{0,2}\s*[:：]\s*'
    r'(?:《[^》\n]{1,80}》|[^\s、，,《\n]{1,60}\.(?:docx|pdf|xlsx|pptx))'
    r'(?:\s*[、，,和与及]\s*(?:《[^》\n]{1,80}》|[^\s、，,《\n]{1,60}\.(?:docx|pdf|xlsx|pptx)))*'
    r'\s*[。.]?\s*$',
    re.MULTILINE,
)
...
def strip_doc_citations(text: Optional[str]) -> str:
    ...
    cleaned = _BARE_SOURCE_REFLIST_PATTERN.sub('', cleaned)  # 裸「来源：」纯引用列表行
