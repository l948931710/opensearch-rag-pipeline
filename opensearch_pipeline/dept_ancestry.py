...
ANCHOR_GROUPS_BY_DEPT_ID: Dict[int, List[str]] = {
    ...
    # —— 海外系重组补锚（2026-08-04）：下面三个单位 2026-08 前挂在 海外中心 下、靠上面那个锚覆盖，
    #    现已【升为顶层树】（dept_id 不变，父从 599944033 变 1）⇒ 祖先链再也够不到海外中心锚。
    #    锚值沿用 2026-07-03 海外系拍板口径（与名字表当时的 获胜工厂/印尼公司/墨西哥公司 逐字相同），
    #    不新增任何组值。⚠️ 获胜系刻意锚在【获胜生产中心】而非树根「获胜包装」：07-03 拍板时
    #    获胜工厂是无子女叶子，今天树根下辖 生产/行政 两条职能线，在根设锚等于把叶子级拍板放大成
    #    89 人双职能授权，违反本表开头的「多职能不在中心设锚」原则（海外子公司的行政/HR 人员
    #    会拿到大陆生产伞组）。两条职能线各自设锚（2026-08-04 拍板），树根仍不设锚：
    #    直挂 11 人维持 fail-closed —— 树根是「公司」语义，职能不明，宁缺勿错。
    1091525269: ["overseas", "production"],  # 获胜包装/获胜生产中心
    1091358296: ["admin"],                   # 获胜包装/获胜行政中心（2026-08-04 拍板：行政线给 admin，
    #     刻意【不】叠 overseas/production —— 行政职能与生产内容无关，伞组只按职能给）
    488554573: ["overseas", "production"],   # 印尼富岭（原 海外中心/印尼公司）
    488669494: ["overseas", "production"],   # 墨西哥富岭（原 海外中心/墨西哥公司）
    # —— 子树内例外（比中心更近 → 天然覆盖，无需特判） ——
    728779788: ["supply", "pmc", "production"],  # 生产中心/资材部（2026-08-04 观测现名「采购部」，
    #     id 不变 ⇒ 锚值不动：改名免疫正是 dept_id 键控的卖点）：权限单 [supply,pmc] + 叠 production（2026-07-03 拍板）
    # —— 多职能中心：锚下沉到二级部门 ——
