...
报告 §7 明确其收敛属 WS2（降低爆炸半径）。DEFER（已注）：chat_stream 流式、提示词模拟
function-calling（DashScope qwen 原生支持 tool_calls）、llm_call_log durable 表（per-run token
预算已由 run_store.consume_budget 覆盖；per-call 成本账本另立 schema）。
"""
...
class DashScopeProvider:
    ...
    def chat(self, model: str, req: ChatRequest) -> ChatResponse:
        ...
        try:
            resp = _http_post(url, json=body, headers=headers, timeout=self._timeout)
...
def make_model_fn(gateway: ModelGateway, ctx: ExecutionContext,
                  category: str = "light") -> Callable[[List[Msg], List[ToolSpec]], ModelTurn]:
    ...
    def _model_fn(msgs: List[Msg], tools: List[ToolSpec]) -> ModelTurn:
        ...
        resp = gateway.complete(ctx, category, req)
        if resp.tool_calls:
            return ModelTurn(
