...
async def _lifespan(_app: FastAPI):
    ...
    # B3（生产级外审 2026-07-17 RB-02）：TLS 接线自检——配了 CA 却明文=接线缺陷，
...
def _rds_tls_startup_check() -> None:
    ...
    try:
        ...
        if cfg.environment not in ("production", "staging") or cfg.simulate_db \
            ...
            return
        from opensearch_pipeline import readiness as _rdn
...
def current_identity(authorization: Optional[str] = Header(None)) -> Optional[Identity]:
    ...
    if uid and os.environ.get("RAG_LIVE_ACL_REREAD", "true").lower() in ("1", "true", "yes"):
        ...
        if strict:
            ...
            if live is not None:
                groups = live
                # F4d×P1-08 统一契约（2026-07-21 迁移批A2，两树同文）：resolver 现把墓碑
                ...
                groups = []
            # else：查询成功但无在册行 → 保留令牌组（未缓存 ≠ 停用）。P1-08 旧 else 分支
            # 的墓碑判定在 F4d resolver 语义下成为死路径（墓碑已由 live=[] 表达），随
            # 迁移批A2 删除；user_row_revoked 的 agent 审批续跑消费点（routes/agent.py）不变。
        else:
...
def _compute_readiness(cfg):
    ...
    # 评审F11（2026-07-21 自 main c7b8723 迁移）：两个可选严格钳——默认只报告，
    # 显式要求时运营库 schema 契约 / 钉钉 Stream 连通纳入摘流量判定。
