...
接线原则：
  ...
  · changed-rows 陷阱（连接池未开 CLIENT_FOUND_ROWS，同值 UPDATE rowcount=0）：
    ...
    终态写 SET 可能全同值的路径（分类持久化）不得以 rowcount 判租，改走
    verify_for_update（行锁 + holder/epoch 相等判定）；
  · 失锁墓碑（main 移植修订，codex 共识 2026-08-02）：**非自愿**失锁（续租/验租/
    栅栏发现被接管）在 LeaseSet 内留墓碑——该 key 后续 fence/verify 一律拒绝
    （raise LeaseLost / 返回 False），「失主文档走到栅栏写时自然弃单」由此被强制，
    不再依赖调用点自觉（op0 版 discard 后栅栏退化 no-op，僵尸后续写全裸）；
    **自愿**释放（终态已落后的 discard()）不留墓碑，释放后的辅助写回归无栅栏旧语义。
    从未注册的 key 维持 no-op legacy 语义不变。off 臂优先级最高：flag 关时一切
    先行早退，运行期残留墓碑不影响 off 臂。
"""
...
class LeaseSet:
    ...
    def __init__(self):
        ...
        self._lost = set()      # type: set  # 非自愿失锁墓碑（见模块头「失锁墓碑」）
    ...
    def fetch_and_register(self, cursor, keys: Iterable[DocVersionKey]) -> int:
        ...
        with self._lock:
            for row in rows:
                ...
                # 同 run 内合法重认领（新 epoch 已到手）⇒ 清墓碑；仅对本次 SELECT
                # 返回的 holder=me 行生效，未重领的 lost 键墓碑保留。
                self._lost.discard((doc_id, version_no))
    ...
    def discard(self, key: DocVersionKey) -> None:
        """**自愿**释放（终态已落，本地放手）——不留墓碑：随后同 doc 的辅助写
        回归无栅栏旧语义（DONE 后的 vlm 覆写等点位依赖此性质）。"""
        with self._lock:
            self._epochs.pop(key, None)
            self._last_renew.pop(key, None)
            self._lost.discard(key)

    def _mark_lost(self, key: DocVersionKey) -> None:
        """**非自愿**失锁（被接管）——弹出台账并留墓碑：该 key 后续 fence/verify
        一律拒绝，僵尸对该 doc 的任何后续写都到不了库。"""
        with self._lock:
            self._epochs.pop(key, None)
            self._last_renew.pop(key, None)
            self._lost.add(key)
    ...
    def fence_where_sql(self, key: DocVersionKey) -> str:
        ...
        墓碑（非自愿失锁）⇒ 先于 execute 抛 LeaseLost——写压根不会发出。"""
        if not lease_enabled():
            return ""
        with self._lock:
            if key in self._lost:
                raise LeaseLost("lease already lost: doc=%s v=%s (fence refused)"
                                % (key[0], key[1]))
            ep = self._epochs.get(key)
        if ep is None:
            return ""
    ...
    def check_fenced_write(self, cursor, key: DocVersionKey, expected_rowcount: int = 1) -> None:
        ...
        ⚠️ 只适用于 SET 恒改行的写（终态翻转/retry 自增）；SET 可能全同值的路径
        （分类持久化）改走 verify_for_update，见模块头 changed-rows 注。"""
        if not lease_enabled():
            return
        with self._lock:
            if key in self._lost:
                raise LeaseLost("lease already lost: doc=%s v=%s (post-write check)"
                                % (key[0], key[1]))
            ep = self._epochs.get(key)
        if ep is None:
            return
    ...
    def verify_still_held(self, cursor, key: DocVersionKey) -> bool:
        ...
        off/未登记 ⇒ True（不介入，语义=现状放行）；墓碑 ⇒ False（已判失锁）。"""
        if not lease_enabled():
            return True
        with self._lock:
            if key in self._lost:
    ...
    def verify_for_update(self, cursor, key: DocVersionKey) -> None:
        ...
        off/未登记 ⇒ no-op；墓碑 ⇒ 直接 raise（已判失锁，不再触库）。"""
        if not lease_enabled():
            return
        with self._lock:
            if key in self._lost:
                raise LeaseLost("lease already lost: doc=%s v=%s (verify refused)"
                                % (key[0], key[1]))
            ep = self._epochs.get(key)
        if ep is None:
            return
