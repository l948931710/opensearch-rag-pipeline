...
async function loadOpsMetrics() {
  ...
  try {
    const r = await apiJson<KbOpsMetrics>('/api/kb/ops-metrics', { auth: true })
    if (fp !== identityFingerprint()) return
    kbOpsMetrics.value = r
  } catch (e) { if (fp === identityFingerprint()) noteLoadError('opsMetrics', e) /* 兜底 */ }
}
...
async function loadFeedbackReview() {
  ...
  try {
    ...
    if (fp !== identityFingerprint()) return   // 身份已切换：旧身份的失败不落错误态
