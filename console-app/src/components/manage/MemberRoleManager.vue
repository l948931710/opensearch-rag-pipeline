...
async function submit() {
  if (!formUser.value.trim() || !formDepts.value.length) { alert('请填 staffId 并至少选一个可管理部门'); return }
  const ok = await grantDeptAdmin(formUser.value.trim(), formName.value.trim(), formDepts.value, formNote.value.trim())
  if (ok) { formUser.value = ''; formName.value = ''; formDepts.value = []; formNote.value = ''; formOpen.value = false }
}
function onRevokeAll(a: AdminItem) {
  if (!confirm(`撤销「${a.user_name || a.user_id}」的全部部门管理权限？\n该用户将降为普通员工（即时失去管理入口）。`)) return
  void revokeAdminGrant(a.user_id, '')
}
function onRevokeDept(a: AdminItem, d: string) {
  if (!confirm(`撤销「${a.user_name || a.user_id}」对【${deptLabel(d)}】的管理权限？`)) return
  void revokeAdminGrant(a.user_id, d)
}
</script>
    ...
    <div class="overflow-hidden rounded-[15px] border border-border bg-card">
      ...
      <div v-if="formOpen" class="space-y-2.5 border-b border-border bg-panel px-[18px] py-3.5">
        <div class="flex flex-wrap gap-2.5">
          <input v-model="formUser" placeholder="钉钉 staffId" class="w-44 rounded-lg border border-border bg-card px-3 py-1.5 text-sm text-foreground placeholder:text-faint" />
        ...
        <div>
          <div class="mb-1 text-[11.5px] text-faint">可管理部门（多选，提交即覆盖该管理员全部授权）</div>
