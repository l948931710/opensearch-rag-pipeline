...
const { kbStats, approvals, kbGovernance, kbInsights, feedbackReview, reviewTasks, loadStats, loadGovernance, loadInsights, loadErrors } = useKb()

// 「待你处理」置顶条（P2，批次α-① 扩为多队列聚合）：页尾 ~2900px 深处实际有多个行动队列——
// 差评复核 / 入库复审（安全网信号），此前 chip 只算差评。
...
// reviewTasks 初始为 null（未加载/加载中）——(x || []) 兜底后计 0，不误亮。
...
<template>
  <div class="space-y-6">
    ...
    <div
      v-if="feedbackOpenCount || feedbackLoadFailed || reviewTaskOpenCount"
