...
<template>
  <!-- 收尾动作：复制 + 转人工 常驻可见（失败兜底不再藏在 hover 后）；赞/踩为次级评价，保留 hover/聚焦/触屏浮现。 -->
  <div class="mt-2 flex items-center gap-0.5 text-muted-foreground">
    ...
    <!-- 次级评价：赞/踩，hover/聚焦/触屏时浮现（opacity 占位，不致布局位移）。 -->
    <div class="flex items-center gap-0.5 opacity-0 transition-opacity duration-150 group-hover/msg:opacity-100 focus-within:opacity-100 [@media(hover:none)]:opacity-100">
