...
3. **编号严格单调递增，且跨分支全局分配**（Majors ε3 纠偏 2026-07-22：本注长期停在
   ...
   （文件与分支侧逐字节一致，checksum 台账不变）。**下一个可用号 = 068**（064/066/067 在 main，
   **065 已被 `feature/doc-version-notify` 占**），取号前先查两侧 `schema/` 目录与分支
...
| 063_visibility_intent.sql | fuling_knowledge | ✅ **2026-08-04 已 apply 生产 + staging**（PROD-RW:2026-08-04；prod 3070 行 / stg 566 行 permission_override 全 NULL ⇒ 行为不变）。C9 方案 B′（Sam 2026-08-03 拍板）：`document_version.permission_override` —— nullable **canonical 权限值**（非布尔位），NULL=无显式意图走 raw_key 路径解析（历史行为逐字节不变），非 NULL=管理员显式声明、stage-2 loader 优先采用（命中 `resolve_permission_level` 优先级 1）。修的是「set-visibility 改完被 stage-2 按 raw_key 覆盖回写」——机械根因是**两个写方锁不相交的行**（端点只锁 document_meta、stage-2 只锁 document_version 的 `FOR UPDATE OF dv SKIP LOCKED`），故本批同时给端点补当前版本行 `FOR UPDATE`，两者才真互斥。**只由显式管理员动作写**——绝不能反过来让 RDS 全局赢（stage-1 自动注册 INSERT 不含 permission_level 列、落默认 'public'，DataWorks 批量注册硬编码 'public'，全局让 RDS 赢会确定性破坏 raw/.../internal/ 与 restricted）。读写两侧**均带 capability 探测**，先部署后 apply 安全（与 048/049/050/062 同款；⚠️ 与 062 不同，本列缺失是**降级跳过**而非报错） |
| 064_content_binding.sql | fuling_knowledge | ✅ **2026-08-04 已 apply 生产**（PROD-RW:2026-08-04，apply_migration.py；3070 行存量全 LEGACY_UNBOUND、raw_version_id 全 NULL ⇒ 行为不变）；**staging 亦已 apply**（2026-08-04 11:48，566 行全 LEGACY_UNBOUND）。C8 审批内容绑定（Sam 2026-08-04 拍板 version-id 固化）：`document_version.raw_version_id` + `content_binding_mode`。修的是**签名 PUT URL 的 TOCTOU** —— put_url 与 upload token 共用 30min TTL 且**预签名 URL 服务端无法逐个撤销**，于是「上传 A → 审批放行 → 同一 url 重 PUT B → 摄取 B」。⚠️ 裸 ETag 复核**不够**（register 存 ETag(B)、审批前 PUT A 给人看、审批后 PUT B 过 If-Match），必须**同时**固定「审批预览读的」与「摄取读的」两处身份 —— version-id 天然满足（重 PUT 产生新 version）。`content_binding_mode` 是**三态显式契约**（codex BLOCKER）：只靠 `raw_version_id IS NULL` 分不出「存量」与「新写失败」，会让绑定静默退化。默认 `LEGACY_UNBOUND` ⇒ apply 后、flag 开前逐字节不变行为（先部署后 apply 安全）|
| 066_document_meta_owner_dept_nullable.sql | fuling_knowledge | ✅ **2026-08-04 23:26 已 apply 生产**（PROD-RW:2026-08-04，apply_migration.py；apply 后 IS_NULLABLE=YES、存量 1938 行 0 NULL ⇒ 既有行不变）。✅ **staging 23:28 亦记台账，但 ALTER 未执行**——幂等守卫判定 `fuling_knowledge_stg` 该列**本就可空**（COLUMN_COMMENT 仍空）⇒ **漂移是生产独有的，staging 当年按权威 DDL 建表**。**现网漂移纠正，非新特性**：`001` 自首个提交起就写 `owner_dept VARCHAR(64) DEFAULT NULL`，而生产实际是 `NOT NULL`（建库时就分歧，因为在此之前每个写入方都给该列赋值，没人踩到）。2026-08-04 **全员上传中断**的根因：node 归属登记（`kb_register` 的 node 分支）是史上第一个往该列写 NULL 的调用方（按 060 的设计，node 模式归属由 `owner_dept_id` 表达、`owner_dept` 置 NULL 以免组码残值命中 legacy 管理员作用域），生产 `STRICT_TRANS_TABLES` ⇒ 1048 ⇒ 500 ⇒ 前端兜底文案。实测佐证：OSS `raw/node-*/` 13 个孤儿对象、`acl_mode` 分布 legacy 1938 / node 0、`kb_doc_node_grant` 0 行。放松约束方向安全（1938 行存量 0 NULL 0 空串，逐字节不变；legacy 作用域腿 `owner_dept IN (…)` 天然不命中 NULL）。⚠️ **事实上不可逆**：一旦有 node 文档登记成功，收紧回 NOT NULL 会 1138。⚠️ 桩库测试抓不到此类"现网约束严于权威 DDL"的缺陷（`_FakeCur` 不校验 NOT NULL）⇒ node 上传端到端用例须落 `tests/test_kb_db_integration.py` |
| 067_contribution_node_axis.sql | fuling_operation | ✅ **2026-08-07 02:57 已 apply 生产**（Sam 当日授权 `PROD-RW:2026-08-07`，经 scripts/apply_migration.py；台账 checksum ea3b9440；MySQL 8.0.36，5 行存量 `category_dept_id` 全 NULL ⇒ 行为不变）；**staging 02:5x 同批已 apply**（`fuling_operation_stg`，2 行存量全 NULL）。列与索引均经 information_schema 复核在册。贡献域归属迁 node 轴（方案 docs/contribution_node_axis_migration_2026-08-06.md，Claude-Codex 四轮 APPROVE）：`kb_contribution.category_dept_id` + `idx_dept_id_status (category_dept_id, review_status)`。修的是「只被 `dept_admin_node_grant` 授权的管理员在贡献域完全失明」——队列/采纳/驳回/重试/通知八个消费点全部只认组码。**轴隔离**：NULL=组码轴行（判定逐字节不变）、非 NULL=node 轴行（只看本列），两支互斥不 OR 交叉（交叉 = node 行的 `category_dept` 残值继续命中旧组码管理员，与 060 的 `owner_dept` 残值越权同型）。⚠️ 组码→dept_id 是**一对多**，代码侧任何位置禁止反推本列；存量迁移只走 ID 白名单 + 执行前重跑数量预检（方案 M8）。读写侧 routes/contribution.py + admin_notify.py 带 capability 探测（1054/1146 → 恒 legacy 分支），**先部署后 apply 与先 apply 后部署都安全**；存量行全 NULL ⇒ apply 后行为不变 |
