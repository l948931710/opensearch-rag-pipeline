...
3. **编号严格单调递增，且跨分支全局分配**（Majors ε3 纠偏 2026-07-22：本注长期停在
   ...
   （文件与分支侧逐字节一致，checksum 台账不变）。**下一个可用号 = 066**（main 已用至 065，
   分支侧至 058），取号前先查两侧 `schema/` 目录与分支 README。历史上有三对编号冲突（002/003/006 各两个文件，见下表）——**不改名**（外部
...
| 065_doc_update_notify.sql | fuling_operation | 文档升版可见范围提醒（doc-update-notify，2026-08-04；设计稿 docs/doc_update_notify_design_2026-08-04_DRAFT.md rev3 + §11 拍板）：新表 `doc_update_event`（每 doc×version 一行，UNIQUE 即防重放水位；status 四态 **HOLD=运维性可回放 vs SUPPRESSED=语义性终态**严格二分——把节流条件落成终态会永久静音，是 R1 稿的缺陷）+ `doc_update_notice`（每 事件×用户×通道 一行；state 含 **SENDING 认领态**防 DataWorks 双实例双发；user_id 只存 staffId 不存姓名）。⚠️ **event 表永不留存删除/永不归档**（discover 的反连接依赖行存在，清行=远古升版重新群发），notice 表 6 个月窗 + 进 `_purge_jobs` 主体擦除；两侧均带 1146 探测降级，**先部署后 apply 安全** |
