...
# ── 自助上传(H5/小程序)的 raw_key 形状识别 ──
# kb_upload.build_raw_key 产出 raw/<owner_dept>/DOC_<ULID26>/<upload_id>/<filename>
# （ULID = 26 位 Crockford base32，去 I/L/O/U）。这类 key 由 /api/kb/register 独占注册——
# 它在【事务+行锁】内分配版本号、按 upload_token 继承 owner_dept/permission_level、并对 public/
# 跨组强制走 PENDING_APPROVAL 审批门。本批量工具若把【PUT 成功但 register 未完成】的孤儿当新文件
# 收编，会硬编码 permission_level='public' 且绕过审批门（见下方 INSERT），把本应 dept_internal/
# 待审批的件直接放成全公司可见。故此处一律跳过自助形状，交还给 register 端（孤儿清理另由 GC 负责）。
_SELF_SERVE_DOC_SEG = re.compile(r"^DOC_[0-9ABCDEFGHJKMNPQRSTVWXYZ]{26}$")

def is_self_serve_raw_key(key):
    """raw/<dept>/DOC_<ULID26>/<upload_id>/<filename> → True（自助上传专属，本工具不得收编）。"""
    parts = (key or "").split("/")
    # 至少 5 段、第 3 段是 DOC_<ULID26>、且末段(文件名)非空（排除目录标记）；
    # 遗留扁平 key（raw/<dept>/<file>，doc_id 不在路径里）与本工具旧 DOC_<DEPT>_<ts> 形状均不命中。
    return (len(parts) >= 5 and parts[0] == "raw"
            and bool(_SELF_SERVE_DOC_SEG.match(parts[2])) and parts[-1] != "")
...
# ═══════════════════════════════════════════════════════════════
new_files = []
self_serve_orphans = 0
for key, size in sorted(oss_files.items()):
    if key not in existing_keys:
        # 自助上传孤儿（PUT 成功但 register 未完成）：绝不收编（否则绕过 register 的可见范围继承
        # 与审批门 → 误放成 public）。交还 /api/kb/register；孤儿存储由 GC 清理。
        if is_self_serve_raw_key(key):
            self_serve_orphans += 1
            continue
        new_files.append((key, size))

if self_serve_orphans:
    print(f"   ⏭️  跳过 {self_serve_orphans} 个自助上传孤儿 key（raw/<dept>/DOC_<ULID>/…，归 /api/kb/register 独占；不在此收编）")
