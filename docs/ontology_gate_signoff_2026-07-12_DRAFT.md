...
**事实确认（签字前请先答，影响标注配额与管家工作量预估，不影响模型结构）**：

- **产品-客户基数**：同一个产品卖给多个客户的情况占比大概是——
  多数产品多客户 ☐ / 多数产品单客户（定制/专模为主）☐ / 大约对半 ☐
  （若「多客户」普遍：GT 的跨命名空间难例配额从 ≥10 行上调至 ≥20 行，
  且客户货号 case 会是管家日常主要工作量来源，gate ② 产能按此预估；
  若业务常需按客户反查产品清单，后续排「客户实体 + 关系边」的正式建模。）
