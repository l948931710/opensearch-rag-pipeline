...
- 返回：**仅调用者自己所在的部门**（`{dept_id, name}` 列表，1-N 个），数据源=`dingtalk_identity` 已有的 `dept_ids`（`dingtalk_identity.py:643`）
- **不返回整棵树，不返回任何管辖/授权字段**
- 前端：1 个 ⇒ 只读展示；多个 ⇒ 下拉选
...
| 情形 | 处置 |
|---|---|
| 归属节点是未授权的 depth1 | fail-closed 落 kb_admin 兜底；长期解法=显式授予该根（Sam 裁决 L1） |
