...
**UI 静默红线(Sam 2026-08-04「不想太多通知影响 UIUX」)**:通知在 console 的唯一形态=**顶栏铃铛 + 点击才展开的下拉面板**,不占任何页面版面;①无未读时零装饰(灰铃铛,无红点无数字);②有未读只出数字 badge(99+ 封顶),**绝不 toast/绝不自动展开/绝不浏览器推送/绝不内容区横幅**;③点空白即收,「全部已读」一键清。铃铛在顶栏(全体用户),不进「管理」tab;最终样式走 ui-iterate 硬门。
