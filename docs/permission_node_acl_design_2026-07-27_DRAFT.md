...
| 门 | 内容 |
|---|---|
| T1 | ⚠️ **只读半 2026-07-28 全绿**:冒号解析 ✅、OR 项 **≥4096 / 127KB 未见上限** ✅、32节点+10owner 组合形态 ✅ ⇒ **规模风险消除**;剩写入半(冒号值成员匹配),脚本 `scratch/probe_node_acl_t1_20260728.py` 待 Sam 授当日 ack |
...
**仅剩未知**:冒号值写入 MULTI_STRING 后能否被**精确成员匹配**(解析通过 ≠ 存得下 + 匹配得上)。脚本已就绪:`scratch/probe_node_acl_t1_20260728.py`(staging 硬断言 + 当日 ack + 一次性 PK + 推送前占用检查 + 不可匹配 permission_level + 自清理 + 前缀/去冒号/整串三类负例)。**ack 由 Sam 设**:

```
RAG_NODE_ACL_T1_ACK=node_acl_t1:$(date +%F) python3 scratch/probe_node_acl_t1_20260728.py
```

### 9.0d T7 flag 现网值(Sam 2026-07-28 控制台确认)

| flag | 现网值 | 对本设计的影响 |
|---|---|---|
| `RAG_ALLOWED_DEPTS_ACL` | **ON** | ✅ allowed_depts 通道**已是活的**(过滤器发 OR 分支、stage-3 推字段、outbox/reconcile 在跑)⇒ 节点值 `d:<id>` 可直接搭现成通道,阶段 A **无需额外开通** |
| `RAG_ACL_ANCESTRY` | **OFF** | 活的用户侧映射是**名字表**(104 条 + 85 车间名快照);锚点表 `dept_ancestry.py` **至今 dark** |

**三条由此上升/下降的风险**:

1. ⚠️ **`_deny_revoked_cross_dept` 是活代码,不是理论风险**。它现在就跑在生产检索路径上,做的是**纯组码相交**(`retriever.py:581-584`),且把 `owner_dept` 不在自有集的命中一律判为 cross-dept(`:537-543`)—— node 授权命中会被它**直接丢弃**。§2.2 第 2 条的改造因此是**阻断性**的:不改,node 文档在生产上根本出不来。
2. ⚠️ **`dept_ancestry.py` 的父链遍历从未在生产跑过**(flag 一直 OFF)。新设计要复用它的遍历机制(`get_parent_id` 注入 / 环检测 / partial 三态)⇒ 等于把一段 **dark code 放到权限判定路径上**,测试与灰度必须按"新代码"对待,不能因"模块早就存在"而降低要求。
3. ✅ **反过来是好消息**:锚点表既然从未生效,实现祖先链时**没有历史行为需要兼容** —— §3.1 担心的"锚点 `[]` 不得截断物理链"退化为纯粹的新代码约束(不是兼容包袱);且 legacy 文档走名字表组码、node 文档走 dept_dim 链,**两套零交互**。

⚠️ **投影先于查询上线**:`ALLOWED_DEPTS_ACL` 已 ON ⇒ 一旦把 `d:` 值写进 `chunk_meta.allowed_depts`,**下一次 stage-3 就会推进 HA3**,哪怕过滤器还没加节点分支。这是**安全的顺序**(数据先行、查询后到,期间无人匹配),但要心里有数。

### 9.0e 组织漂移实测 —— 现网已有 281 人只剩 public(2026-07-28)

Sam 确认"钉钉组织最近改过,按新的来"。按 **LIVE 口径**(`RAG_ACL_ANCESTRY`=OFF ⇒ 名字表 + 85 名车间快照)重放全员:

```
全员 1175 人 → fail-closed 仅 public = 281 人(23.9%)
  其中 70 人(「其他」部门)= 有意仅 public,非缺陷
  ⇒ 真实缺陷面 211 人

生产中心快照漂移:硬编码 85 名 vs 实测 72 名
  缺失 7 个活部门(62 人):薄膜车间/薄膜—仓管/其他/切袋/吹膜机修/机修(47人)+ 采购部(15人)
    根因① 「吹膜」整体改名为「薄膜」(6 个部门连带失配)
    根因② 采购部是新部门,全仓无任何映射
  死键 20 个(含 吸塑一、四车间* 系列)—— 无害
  「纸浆模塑事业部」已迁出生产中心 → 现挂 获胜包装/获胜生产中心
```
...
| 群体 | 人数 | 修法 |
|---|---|---|
| 开 `RAG_ACL_ANCESTRY` 即修复 | **154** | 锚点表**早已写好+测过、一直 dark**;flag 一开即生效 |
| 「获胜」系改名(获胜工厂→获胜包装/获胜生产中心/获胜行政中心) | 57 | 加锚 or 更新名字表 |
| 印尼公司→印尼富岭、墨西哥公司→墨西哥富岭 | 2 | 同上 |
| 「其他」部门 | 70 | 有意仅 public,不修 |

⚠️ **这是用户侧缺陷,与文档侧无关** —— Sam 计划的"清空语料 + 各部门重传"**修不掉它**;不修的话重传后这 211 人依然读不到新文档。
✅ node-ACL 从根上消灭这一类:祖先链按 dept_id 走活的组织快照,改名免疫、新部门自动继承,**不再有任何名字快照需要维护**。

**旁证**:现网 `user_role` 仅 **57 行**(55 employee + 2 kb_admin)/ 1175 人 ⇒ seeded 行覆盖面极小,281 这个数字基本就是真实值。
