...
**可选加固**:开**选项 E**(`b5aae22`,漏斗判决落 RDS、已判过的图免疫 VLM 制度漂移)—— 正是大批量重跑的对症药。

~~⚠️ 前置:`schema/059_image_funnel_verdict.sql` **生产尚未 apply**(2026-07-28 查 `schema_migrations` 确认),且默认 OFF。~~
✅ **2026-07-31 更正**:059 早在 **2026-07-29 14:35:24 就已 apply 生产**(见 §9.0f),本行是 07-28 的旧话、apply 后忘了回填,已于 2026-07-31 只读复核现网更正(`image_funnel_verdict` 表在,判决行数 0)。⚠️ **真正卡住的不是 schema,是开关**:`RAG_FUNNEL_VERDICT_STORE` 默认 OFF ⇒ 表建好了但一行判决都不写,加固等于没开。且它与 `RAG_VLM_DOC_CONTEXT` **互斥**(后者把标题塞进 prompt,判决就不再是图片字节的纯函数)。开之前须核实**漏斗的实际执行方**(DataWorks 摄取节点)两个 flag 的真实值 —— 仓库里 `dataworks_nodes/` 未见任何一个,故按 [[flag 传播看执行路径]] 的教训,不能从代码默认外推。
...
### 9.0g 组织快照首次灌入生产(2026-07-31,Sam 授权 + 当日 PROD-RW)—— **节点通道由此点亮**

`org_sync.main(["--commit"])`，从本机跑（钉钉只读 → 写生产 `dept_dim`/`staff_dim`），耗时 573s。

```
[COMMIT] rev=1  部门 119  员工 1173
  新增部门 119  消失部门 0  新增员工 1173  离场员工 0  孤儿授权 0
```

**只读复核（不采信脚本自报，直接查库）**：

```
dept_dim   119 行 / is_active=1 119      staff_dim  1173 行 / is_active=1 1173
snapshot_rev = 1     快照年龄 0h         无部门归属的员工 0
根节点(一级部门) 17                       最大树深 5
其它表零触碰: acl_mode='node' 0 · kb_doc_node_grant 0 · 活跃已索引 chunk 27484(不变)
```

★ **三个结构不变量与 2026-07-28 §9.0b 那次【独立的】钉钉只读实测完全一致**：
部门 119 ✅ / 一级部门 17 ✅ / 树深 5 ✅；员工 1175→1173（−2，三天人员流动）。

**功能验证（灌之前这一列恒为 False）**：

```
staff 直属=734515621             node_channel_ok=True  祖先链 4 跳
staff 直属=33952854              node_channel_ok=True  祖先链 2 跳
staff 直属=735658975,735974634   node_channel_ok=True  两条链正确取并集
staff 直属=735056635,735115529   node_channel_ok=True  5 项并集
→ 该身份的 HA3 过滤器节点项: ['d:734515621','d:731880290','d:899606803','d:599318766','dx:734515621']
```

链长 2–5，与 §9.0b 实测「最大 8 · P99 5 · 平均 2.9」吻合。

⚠️ **此刻仍不影响任何检索行为**：`RAG_NODE_ACL_GRANT` 未开、生产 0 篇 node 文档 ⇒
节点分支不产生任何候选。本次写入解锁的是**管理台侧**（组织选择器有数据、授权端点的
`dept_dim.is_active=1` 校验能过），以及奖励统计的在册人数分母。

⚠️ **快照会过期**：`dingtalk_identity` 按 `TIMESTAMPDIFF(HOUR, MAX(synced_at), NOW())`
判新鲜度，**超 48h 即整条节点通道 fail-closed**。因此 **DataWorks 每日同步节点是必须接的**
（T6 定的每日一次），否则两天后节点授权自动失效。当前是**一次性手工灌入，尚未接调度**。
