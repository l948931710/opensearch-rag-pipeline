...
**只读复检 #1（10:11–10:30，scratch/verify485_heal_20260712.py，prod_ro RDS+公网 HA3）**：
- **全量重推未跑**：今日 chunk 被触碰 1/485（=探针 doc）；document_version 今日
  SUCCESS 收尾 1/485；**双 active 残余 484/485**（探针 doc 的旧版本已被 stage-3
  node05 supersede 自愈——481ed79 版本收尾在真跑里生效的实证）。
- **人群仍盲**：seeded（20260712）抽样 15 docs 首 chunk 自查询（top-10、owner_dept
  臂）**0/15**——与法证同法同值，485 未治愈。
- ✅ **探针 doc 延迟复验通过**：重推（≤09:54）后 ~20+min 自查询 present=1/1
  ——延迟丢失签名暂未出现，全量重推绿灯继续有效。
- ⚠️ **引擎自行漂移 + 双臂分化（工单必备素材）**：工单样本行 id=83138 宿主
  5A271A——法证时 BM25 文号探针（FL-QC-015-002）缺席，现在**无人重推却能召回**
  （倒排臂行级自恢复，期间唯一写入是另一 doc 的单行探针推送）；但该 doc 44/44
  chunk 的 dense 自查询 **top-50 全 miss**（QUALITY 兄弟 doc 刷满结果）→ **dense
  臂仍缺行、served 混检路径仍盲**；另一宿主 13092D 的 BM25 仍缺席（恢复是行级
  零星的）。⇒ 教义再收紧：**BM25/点查命中≠served 可检索，终验只能 dense/hybrid
  自查询**；「引擎后台在动索引、双臂状态不一致」写进工单。
