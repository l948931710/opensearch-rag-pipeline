...
#### 🔴 定值有答案了 —— 而且结论是**压测不必要**（2026-08-06）

先做了两件不碰云资源的：

**① 离线扫描**（`scratch/pool_sizing_sweep_20260806.py`，桩上游、参数化耗时 L=50ms）：

| 并发 | fusionW | 准入cap | p50 | p95 | 拒绝 | 错误 | 峰值线程 |
|---:|---:|---:|---:|---:|---:|---:|---:|
| 30 | 90（默认） | 关 | 0.057 | 0.071 | 0 | 0 | 122 |
| 30 | 12 | 关 | 0.326 | 0.433 | 0 | 0 | 44 |
| 60 | 180（默认） | 关 | 0.063 | 0.096 | 0 | 0 | 242 |
| 60 | 12 | 关 | **0.642** | 0.806 | 0 | 0 | 74 |
| 60 | 12 | 24 | 0.058 | 0.128 | **322** | 0 | 74 |

三条读出来的：
- **限 worker 但不开准入闸 = 最坏组合**（60 并发 p50 涨 10×，全在排队）⇒ 两个旋钮必须一起定；
- 所有开闸行 **错误=0** ⇒ 饱和确实走降级不抛异常（这补上了我此前说"没写"的端到端饱和证据）；
- 🔴 **p50 在开闸后"变好"是假象** —— 60/12/24 那行 322 次拒绝 ≈ 89% 的臂被拒，
  多数查询拿到零臂、回落服务端混合。**快是因为没干活**。
  ⇒ 只看延迟指标会把召回塌方读成性能改善，`rejected` 不接告警就是静默事故。

**② 上游耗时**：笔记本→公网 HA3 量到单臂 p50 **1983ms**，但 WAN 基线没测出来（TCP 握手全失败），
分不开 RTT 与真实耗时 ⇒ **该数据不可用于定值**，据此定会偏大一个量级。已废弃。

**③ 改用生产真实遥测**（`qa_session_log.retrieval_latency_ms`，prod-ro 只读）——
比压测更好：真实查询/ACL/文档规模/网络位置。压测只能造"我以为的负载形状"。

```
近 30 天 n=207（214 条里仅 7 条 latency 为 NULL ⇒ 近乎全集，非抽样）
in-VPC 检索耗时  p50=655ms  p95=1515ms  p99=2042ms  max=2935ms
峰值每分钟       3 条/分 = 0.05 QPS
同秒并发         30 天内**从未超过 1**
全表 1250 条 / 71 天（2026-05-27 ~ 08-06）
```

**Little's law**：fusion 并发 = 0.05 QPS × 1.5s × 3 臂 ≈ **0.23** ⇒ **1 个 worker 就够**。

#### 定值结论

| 项 | 定值 | 理由 |
|---|---|---|
| `RAG_POOL_*_WORKERS` | **维持默认**（AnyIO 令牌 × 臂数） | 它是**上界**不是工作值——`ThreadPoolExecutor` 按需建线程，1 并发下只会建 3 个。默认无害，且给未来留了闸 |
| `RAG_POOL_ADMISSION` | **保持关** | 0.05 QPS 下永不触发；开它是纯风险无收益 |
| staging 压测 | **不做** | 见下 |

**为什么不做压测**（这条是判断，不是偷懒）：
1. 要压的目标是「120 并发 × 3 臂 = 360 线程」，而生产真实并发峰值是 **1**——
   要造出那个场景需要 **1000×** 当前流量，压的是一个不存在的形态；
2. 🔴 `.env.staging` / `.env.prod_ro` / `.env.production` 指向**同一台 RDS 实例**
   （`rm-bp15j7wekd5738f093o`）和**同一个 HA3 实例**（`ha-cn-kgl4slr1n01`），只有库名/表名不同；
   且**仓内没有 staging 的 serving 部署** ⇒ "staging 压测"实际是拿笔记本对
   **生产共享的物理实例**造持续负载。为一个不存在的负载形态承担这个风险，不划算；
3. 真正缺的两个数（Q 与 L）已由生产遥测给出，比压测更准。

⇒ **§5.2 的严重度需下调**：代码形态问题属实（无进程级闸、无指标，已修），
但「360 线程耗尽资源」在当前流量下**不可达**，不是现网风险。

#### 重新定值的触发条件（写死，免得"以后再说"）

任一成立即回来重跑本节：
- 峰值分钟条数 > 60（= 1 QPS，比现在高 20×）；
- 检索 p95 > 3s；
