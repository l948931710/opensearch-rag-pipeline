...
## 4b. 预览积分权重（Sam 2026-08-01 拍板，修订记录）

贡献页「总积分榜」上线预览口径（**展示激励用，非发奖依据**）：

```
score = 10 × 采纳入库(⑦) + 1 × 被引用(cited 事实表,全期) + 3 × 有效反馈(④⑤⑥,限渠道①)
```

- 活跃/提问量**不进**个人分（§0 的结论：量化消耗≠价值）；
- 权重单一来源 = `opensearch_pipeline/routes/contribution.py` 的 `HERO_SCORE_W_*` 常量；
- UI 明标「预览口径」，行内展示构成明细（采纳×N·引用×M·反馈×K）保持可审计；
- 被引用算不出（fact 路径关）时分数按 0 如实少算，hits 显示「—」不用 0 顶替；
- **正式积分换算仍以办法定稿为准**——二稿落地若权重变更，改常量一处 + 在本节追加修订行。
