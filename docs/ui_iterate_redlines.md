...
| 台账词表 seam 做构建期 codegen | 不做——ε-5 R2 双侧枚举+镜像断言已把词表漂移锁成 CI 红，codegen 消灭的失败模式与测试同一个、只添构建链复杂度；若将来出现第三消费方（如小程序同词表），正解=**后端吐稳定码+各端自映射文案**，仍非 codegen | 2026-07-15 |
| 贡献空态出窗免责副题语气 | 维持现文案——免责句是硬约束产物（qa_session_log 无 hash 列，出窗未答计数不可行），删除会复发「空态=都解决了」误读；措辞微调仅在用户给出偏好说法时顺手带走，不单开轮 | 2026-07-15 |
| 管理员作者 failed 行「重试+修改重交」双钮动线 | 不做——双重罕见交集（管理员本人投稿且失败）且两钮语义各自合法（重试=原样重新排队 vs 修改重交=改稿开新单）；上限=各加一句 hint，归打磨候选池不专项 | 2026-07-15 |
| 「忽略此缺口」权限与形态 | 拍板 **交给 dept_admin**（console 管理员级，员工 DOM 零变化）；忽略成功=**行内翻转不重拉**（reload 摘除会让误点永久不可寻回——后端无已忽略列表端点）；非破坏性软删 → 弹窗**不用 danger 红色态**；撤销窗口=至下次 loadGaps（如实语义，非 bug）；「已忽略列表」视图=后续增量待人判断 | 2026-07-15 |
