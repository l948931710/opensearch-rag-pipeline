...
| 2 | [`implementation-plan.md`](implementation-plan.md) | **实施改造计划**：WS0–WS5 文件级改动清单 + 迁移编号 + flag + 测试 + 回滚 + 验收（开 worktree 从这里拆 issue） |
| 3 | [`report-v2-changelog.md`](report-v2-changelog.md) | v1→v2 逐条变更（42 条：确认/推翻/重写/弱化/新增，附双证据） |
...
第一优先级是 **WS0（状态外置与多实例化）**——解除 `--workers 1`、Redis 基建、CD 流水线，是一切 Agent 功能的地基。详见 `implementation-plan.md`。
