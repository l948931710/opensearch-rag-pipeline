...
- **Run**: eval_refreeze_20260727  |  **Table**: `?`  |  **Generated**: 20260727_160518
...
| answer keyword-coverage (L3, 完整性) | >= 0.70 | 0.7152 | ✅ PASS |
...
| <<IMG:N>> marker validity (L4-srv) | >= 0.95 hard / >= 0.98 soft | 1.0 | ✅ PASS |
| dangling 口惠图但卡片无图 (L4-srv) | <= 0.05 hard (扩正则后) | 0.0286 | ✅ PASS |
| orphan rate (L4-srv, trend 监控) | <= 0.30 soft (trend, advisory) | 0.4182 | ❌ FAIL |
| marker distinctness (L4-srv, advisory) | advisory — 1.0 = no image reuse (lower = bundled/reused markers) | 0.8312 | ❌ FAIL |
| answer image rate (L4-srv, advisory) | advisory — trend (锁分布后再定阈值; 主臂=生产 cosurface=False 姿态) | 0.8214 | ➖ N/A |
...
| judge inter-rater agreement (L6 chunk panel) | mean overall inter-judge stdev <= 1.2 (1-5 scale) | 0.2 | ✅ PASS |
| answer faithfulness (Claude, L3) | >= 4.0 / 5 | 4.938 | ✅ PASS |
| answer correctness (Claude, L3) | >= 4.0 / 5 | 4.462 | ✅ PASS |
| answer completeness (Claude, L3) | >= 4.0 / 5 | 4.218 | ✅ PASS |
| positive fabrication (Claude, L3) | <= 0.05 | 0.009 | ✅ PASS |
| negative fabrication (Claude, L3) | <= 0.10 | 0.0 | ✅ PASS |
| judge inter-rater agreement (L3 panel) | mean overall inter-judge stdev <= 1.2 (1-5 scale; lower=more agreement) | 0.104 | ✅ PASS |
| image relevance (judge-mm, advisory) | >= 3.5 advisory（caption 贴题率;先锁两轮分布再议升 hard） | 3.886 | ✅ PASS |
...
| baseline regression (regime) | baseline regime must match run regime to compare | REGIME MISMATCH on ['l6_evaluator_version', 'l1_matcher_version', 'l4_serving_evaluator_version'] — baseline not comparable | ➖ N/A |
...
- latency (ms): {"mean": 4400.7, "p50": 4346, "p90": 5215, "p95": 5422, "p99": 5955}
...
```json
{
 ...
 "mean_overall_interjudge_stdev": 0.104,
 "pass_rate_overall_ge4_positives": 0.809
}
```
...
```json
{
 ...
 "answer_image_rate": 0.8214285714285714,
 "interleave_rate": 0.8214285714285714,
 "orphan_rate": 0.41818181818181815,
 "orphan_rate_all": 0.5761589403973509,
 "n_available_visible": 110,
 "n_available_all": 151,
 "marker_validity": 1.0,
 "marker_distinctness": 0.8311688311688312,
 "dangling_ref_rate": 0.02857142857142857,
 "over_cap_rate": 0.32142857142857145,
 "placement_rate": 0.8214285714285714,
 "avg_images_shown": 3.25,
 "total_markers": 77,
 "total_invalid_markers": 0,
 "total_inrange_markers": 77,
 "total_distinct_markers": 64,
 "n_referenced_none_strategy": 5,
 "n_appended_strategy": 5
}
```
...
- fingerprint: {"active_chunks": 27484, "active_docs": 1481, "chunk_id_set_hash": "a8752660cb5ed728", "d7_json_hash": "62f2476c2b048c99", "code_commit": "cc30033", "rubric_version": "chunk_rubric_v1"}
...
```json
{
 ...
 "by_chunk_type": {
  ...
  "visual_knowledge": {
   ...
   "pass_rate_overall_ge4": 0.333
  }
 },
 "mean_overall_interjudge_stdev": 0.2,
