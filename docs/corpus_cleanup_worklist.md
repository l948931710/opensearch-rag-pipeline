...
## 7. 双向独有内容孪生组 → 文控源文件补全（2026-06-12 第三批退役后遗留）

41 组同名异扩展孪生已退役 37 组（三批 324 chunks，双侧验证全过，工具/journal 在
scratch/）。以下 4 组经内容包含性对比（scratch/twin_content_coverage_20260611.md）
确认 **pdf 与 docx 双向都有真实独有内容**，chunk 级无法安全退役任何一侧——需文控
提供合并后的完整版（或确认其中一侧为权威版本），新文件走 DAG1→3 正常入库后再退役两旧身份：

| stem | pdf 独有（举例） | docx 独有（举例） |
|---|---|---|
| FL-PMC-WI-010《袋子不干胶采购下单》 | 单价/数量/计划到货日期三条填写说明 | 表单字段明细 |
| FL-PMC-WI-013《注塑-商检跟踪表更新》 | "待检库结存数列转数字"等操作步骤 | 产品编码表数据行 |
| FL-XS-WI-005《吸塑领料申请单打印》 | "打开U8/输入密码登录"步骤 + 表单数据 | 产品规格表 + 多 5 张图 |
| FL-ZS-WI-009《注塑发货拖柜》 | 装箱/集装箱单证表格 | 监装毛重±5% 责任条款变体 |

在此之前 4 组双身份继续并行服务（serving 端有标题去扩展名折叠，答案层无重复来源）。
