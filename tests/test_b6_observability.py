...
def test_posture_reports_ops_alert_webhook(monkeypatch):
    from opensearch_pipeline import readiness
    readiness._reset_cache()
    monkeypatch.delenv("RAG_OPS_ALERT_WEBHOOK", raising=False)
    assert readiness.security_posture_report()["ops_alert_webhook"] == "missing"
    monkeypatch.setenv("RAG_OPS_ALERT_WEBHOOK",
                       "https://oapi.dingtalk.com/robot/send?access_token=x")
    assert readiness.security_posture_report()["ops_alert_webhook"] == "configured"
