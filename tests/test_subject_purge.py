...
def test_dry_run_counts_without_deleting(monkeypatch, live_db):
    ...
    assert set(rep["tables"]) == {
        ...
        # agent_tool_operation（schema/045，PR-3 Stage C）回执含主体字段，run_id 归属链
        "agent_checkpoint", "agent_step", "tool_invocation", "agent_tool_operation",
        "llm_call_log", "agent_dispatch_command", "agent_run",
...
def test_commit_deletes_fact_rows_before_session_log(monkeypatch, live_db):
    ...
    # 12 表链：agent 7 表（checkpoint/step/invocation/tool_operation/llm_call/
    # dispatch_command/run）在前，qa 5 表在后（PR-3 Stage C 加 agent_tool_operation，索引 3）
    conn = _Conn(affected=3, act_rowcounts=[0, 0, 0, 0, 0, 0, 0, 3, 0, 0, 0, 3])
    monkeypatch.setattr("opensearch_pipeline.db._get_db_conn", lambda *a, **k: conn)
