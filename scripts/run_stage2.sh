#!/bin/bash
# ==============================================================================
# run_stage2.sh — 手动触发 Stage 2 (Canonical -> Safe Chunks)
# ==============================================================================
# 用法：
#   ./scripts/run_stage2.sh             # 默认使用当天日期 (YYYYMMDD)
#   ./scripts/run_stage2.sh 20260520    # 处理指定业务日期的文档
# ==============================================================================
...
# 3. 打印提示信息
echo "🚀 开始手动执行 Stage 2: Canonical -> Safe Chunks (分类 + 脱敏 + 切分 + 写入元数据)..."
echo "📂 业务日期: $BIZDATE"
echo "--------------------------------------------------"
...
if [ $EXIT_CODE -eq 0 ]; then
    echo "✅ Stage 2 执行成功！"
    echo "👉 下一步您可以运行 Stage 3 的命令或脚本进行向量生成与索引推送。"
else
    echo "❌ Stage 2 执行失败！请检查上方输出的错误日志。"
    exit $EXIT_CODE
fi
